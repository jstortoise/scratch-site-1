				...
				<?php
					...
					if ($result->num_rows > 0) {
						...
						while ($row = $result->fetch_assoc()) {
							echo "<tr><td>". $row["TransactionID"]. "</td><td>". $row["DocumentNo"]. "</td><td>". $row["Name"]. "</td><td>". $row["Address"]. "</td><td>". $row["Telephone"]. "</td><td>". $row["Remarks"]. "</td><td>". $row["Date"]. "</td><td>". $row["Tax"]. "</td><td>". $row["TotalPrice"]. "</td><td>". $row["DiscountAmt"]. "</td><td>". $row["AmountPaid"]. "</td><td>". $row["ChangeAmt"]. "</td><td><a href='index.php?page=viewItems&unique=". $row["TransactionID"]. "'>Items</a></td><td><a href='editrowtrans.php?unique=". $row["TransactionID"]. "' onClick=\"window.open(this.href,'edit','height=646,width=310'); return false;\">Edit</a></td><td><a href='delrowtrans.php?unique=". $row["TransactionID"]. "' onClick=\"window.open(this.href,'delete','height=161,width=160'); return false;\">Delete</a></td></tr>";
						}
					}
					$conn -> close();
				?>
